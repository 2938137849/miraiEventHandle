...
/**
 * 封装事件处理
 * @property plugin JvmPlugin 插件自身
 * @constructor
 */
class MiraiEventHandle(private val plugin: JvmPlugin) {
	/**
	 ...
	 * @param obj [Any] 任意对象
	 */
	operator fun plus(obj: Any): MiraiEventHandle {
		register(obj)
		return this
	}
	...
	private fun KCallable<*>.MiraiEventHandle() = annotations.filterIsInstance<EventHandle>().firstOrNull()

	private fun AnnotatedElement.MiraiEventHandle() = annotations.filterIsInstance<EventHandle>().firstOrNull()
}
